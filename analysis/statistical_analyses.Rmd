...
```{r exp1stats, include=TRUE, fig.height=4, fig.width=6}
...
d1_stats1 <- d1 %>% 
  mutate(ri = factor(case_when(response=="RC_IC" ~  NA_character_,
         ...
         #lengthc = sdif_coding(factor(length))
         lengthc = contr.sum(factor(length))  # use sum coding
        )
...
fit1_ri <- brm( ri ~ state_or_event + lengthc + (1|participantnum), family=bernoulli(), data = d1_stats1, chains = nchain)

fit1_cd <- brm( cd ~ state_or_event + lengthc + (1|participantnum), family=bernoulli(), data = d1_stats1, chains = nchain)
