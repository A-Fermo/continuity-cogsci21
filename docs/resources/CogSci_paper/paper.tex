...
To support these two conclusions we ran two Bayesian mixed effects regression models using the \emph{brms} package in R~\cite{brms} with default priors\footnote{By default brms uses improper flat priors on the coefficients of fixed effects, and a half student-t prior with 3 degrees of freedom, a location of 0 and a scale of 2.5 on the intercept and on the standard deviations of the random effects.}. For the first model, each response was coded as root or immediate, and all responses that could not be classified in this way (including all responses for short sequences) were discarded. We then ran a logistic regression in which stativity (i.e.\ state vs event) and network length (long vs medium) were included as predictors of a binary dependent variable (root vs immediate). We included a random intercept for participant to allow for statistical dependencies between multiple responses from the same participant. The posterior mean of the intercept variable was 10.54 and the 95\% credible interval was $[2.30, 26.54]$, which supports the conclusion that root causes were chosen more often than immediate causes. The credible intervals for the other coefficients both included 0 (state: -2.75, $[-10.03, 1.14]$ ; long: 1.85, $[-1.78, 7.33]$), suggesting that the preference for root causes was not strongly affected by either stativity or network length. 
